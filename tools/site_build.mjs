...
if (html.includes('importmap') || html.includes('src/main.js')) throw new Error('index.html rewrite failed');
...
for (const f of ['posts.json', 'tv.mp4', 'og.jpg']) if (fs.existsSync(`${SRC}/assets/${f}`)) copy(`${SRC}/assets/${f}`, `${OUT}/assets/${f}`);
fs.copyFileSync(`${SRC}/posthog.js`, `${OUT}/posthog.js`);
